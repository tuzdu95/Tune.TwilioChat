var twiliochat = (function () {
    ...
    tc.addMessageToList = function (message) {
        tc.currentMessages.push(message);
        createChatBubble(tc.currentMessages, message, tc.currentMessages.length - 1);
    ...
    function createChatBubble(messages, message, index) {
        var rowContainer = $('<div>').addClass('col chat-wrapper');
        rowContainer.attr('id', `${index}_container`);

        var rowDiv = $('<div>').addClass('row no-margin');
        rowDiv.attr('id', `${index}_message`);

        const isAuthor = message.author === tc.username;
        if (index > 0) { // check if same author as previous author to remove image and date
            var lastItem = messages[index - 1];
            var prevDate = `#${index - 1}_date`;
            var prevAuthor = `#${index - 1}_author`;
            var prevMessage = `#${index - 1}_message`;
            var prevContainer = `#${index - 1}_container`;
            if (message?.author == lastItem.author) {
                //rowContainer = $(prevContainer);
                $(prevDate).addClass('no-display');
                $(prevAuthor).addClass('no-display');
                $(prevMessage).addClass(`${isAuthor ? 'margin-right-chat' : 'margin-left-chat'}`);
            } else {
                rowDiv.addClass('gap');
            }
        }
        ...
        date.innerText = dateFormatter.getTodayDate(message.dateCreated);
        date.id = `${index}_date`;

        var author = document.createElement('img');
        author.src = 'https://i.pinimg.com/736x/1c/82/c5/1c82c5abe0a69642e34db49683dd3734.jpg';
        author.className = 'chat-icon';
        author.id = `${index}_author`;

        if (isAuthor) {
            rowDiv.append(body);
            date.className = 'message-date message-date-right';
            rowDiv.append(date);
            rowDiv.addClass('msg');
            rowDiv.addClass('sent');
            rowContainer.append(rowDiv);
            rowContainer.append(author);
        }
        else {
            rowDiv.append(body);
            date.className = 'message-date message-date-left';
            rowDiv.append(date);
            rowDiv.addClass('msg');
            rowDiv.addClass('rcvd');
            rowContainer.append(author);
            rowContainer.append(rowDiv);
        }
        tc.$messageList.append(rowContainer);
    }
